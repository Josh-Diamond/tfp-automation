...
func (t *TfpSanityTestSuite) TfpSetupSuite() (*config.TerraformConfig, *config.TerratestConfig) {
	...
	return t.terraformConfig, t.terratestConfig
}
...
func (t *TfpSanityTestSuite) TestTfpProvisioningSanity() {
	...
	for _, tt := range tests {
		terraformConfig, terratestConfig := t.TfpSetupSuite()
		...
		t.Run((tt.name), func() {
			...
			configMap := []map[string]any{t.cattleConfig}

			clusterIDs := provisioning.Provision(t.T(), t.client, t.rancherConfig, terraformConfig, terratestConfig, testUser, testPassword, clusterName, poolName, t.terraformOptions, configMap)
